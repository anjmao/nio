...
func TestJWTRace(t *testing.T) {
	...
	makeReq := func(token string) nio.Context {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		res := httptest.NewRecorder()
		req.Header.Set(nio.HeaderAuthorization, DefaultJWTConfig.AuthScheme+" "+token)
...
func TestJWT(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		res := httptest.NewRecorder()
		req.Header.Set(nio.HeaderAuthorization, tc.hdrAuth)
		req.Header.Set(nio.HeaderCookie, tc.hdrCookie)
		c := e.NewContext(req, res)

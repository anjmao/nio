...
type (
	...
	JWTSuccessHandler func(nio.Context)

	// JWTErrorHandler defines a function which is executed for an invalid token.
	JWTErrorHandler func(error) error

	jwtExtractor func(nio.Context) (string, error)
)
...
// Errors
var (
	ErrJWTMissing = nio.NewHTTPError(http.StatusBadRequest, "missing or malformed jwt")
)
...
func JWT(key interface{}) nio.MiddlewareFunc {
	c := DefaultJWTConfig
	c.SigningKey = key
...
// See: `JWT()`.
func JWTWithConfig(config JWTConfig) nio.MiddlewareFunc {
	// Defaults
	if config.Skipper == nil {
		config.Skipper = DefaultJWTConfig.Skipper
	}
	if config.SigningKey == nil {
		panic("nio: jwt middleware requires signing key")
	}
